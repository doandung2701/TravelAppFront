    ...
    return (
      <div>
        <h2 id="location-heading">
          <Translate contentKey="travelwebApp.location.home.title">Locations</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="travelwebApp.location.home.createLabel">Create new Location</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {locationList.map((location, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{location.providerName ? <Link to={`provider/${location.providerId}`}>{location.providerName}</Link> : ''}</td>
