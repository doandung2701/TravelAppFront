...
export class Hotel extends React.Component<IHotelProps> {
  ...
  render() {
    ...
    return (
      <div>
        <h2 id="hotel-heading">
          <Translate contentKey="travelwebApp.hotel.home.title">Hotels</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="travelwebApp.hotel.home.createLabel">Create new Hotel</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {hotelList.map((hotel, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{hotel.address}</td>
                  <td>{hotel.phone}</td>
                  <td>{hotel.providerName ? <Link to={`provider/${hotel.providerId}`}>{hotel.providerName}</Link> : ''}</td>
