    ...
    return (
      <div>
        <h2 id="comment-heading">
          <Translate contentKey="travelwebApp.comment.home.title">Comments</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="travelwebApp.comment.home.createLabel">Create new Comment</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {commentList.map((comment, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{comment.customerName ? <Link to={`customer/${comment.customerId}`}>{comment.customerName}</Link> : ''}</td>
                  <td>{comment.tourName ? <Link to={`tour/${comment.tourId}`}>{comment.tourName}</Link> : ''}</td>
